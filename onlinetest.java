...
class OnlineTest extends JFrame implements ActionListener {
	...
	synchronized void set() {
		...
		if (current == 2) {
			try {
				Thread.sleep(10);
			label.setText("Q3:  Which provides runtime environment for java byte code to excute?");
		...
		if (current == 1) {
			label.setText("Q2: Which of the following is not a Java features.");
		...
		if (current == 3) {
			label.setText("Q4:  In which process, a local variable has the same name as one of the instance variables?");
		...
		if (current == 6) {
			label.setText("Q7: Which package contains the Random class?");
		...
		if (current == 5) {
			label.setText("Q6:  In java, jar stands for_____?");
		...
		if (current == 8) {
			label.setText("Q9:  Which one is a template for creating different objects?");
		...
		if (current == 7) {
			label.setText("Q8: Which of the following are not Java keywords.");
